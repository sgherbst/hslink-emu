...
class LFSR:
    def __init__(self):
        self._poly_dict = {}
        for key, val in self._poly_src.items():
            taps = val.split(',')
    ...
    def write_package(self, dir_name, name='lfsr_package'):
        ...
        # create an array of the number of taps required
        n_taps_array = []
        for k in range(n_max+1):
            if k in self.poly_dict:
                n_taps_array.append(len(self.poly_dict[k]))
            else:
                n_taps_array.append(-1)

        # determine the maximum number of taps for any output width
        n_taps_max = max(n_taps_array)

        # create 2d array of taps for each setting
        taps2d = []
        for k in range(n_max+1):
            # get taps for this output width
            if k in self.poly_dict:
                taps = self.poly_dict[k]
            else:
                taps = [-1] # filler

            # pad out taps to the maximum length needed
            taps += [-1] * (n_taps_max-len(taps))

            # add result to 2d array
            taps2d.append(taps)
